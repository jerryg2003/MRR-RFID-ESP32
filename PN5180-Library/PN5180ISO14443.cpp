...
int8_t PN5180ISO14443::readTypeA(uint8_t *buffer, uint8_t kind, bool allFunc) {
	uint8_t cmd[7];
	uint8_t uidLength = 0;


if (allFunc)         {	             // *** Selectively, remove some functions and see if still works 2023-06-13
// OFF Crypto
	if (!writeRegisterWithAndMask(SYSTEM_CONFIG, 0xFFFFFFBF)) {
		PN5180DEBUG(F("*** ERROR: OFF Crypto failed!\n"));
		return -1;
	}
}     // *** Selectively, remove some functions and see if still works 2023-06-13
