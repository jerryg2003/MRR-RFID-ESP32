...
bool RFIDReader::ReadRFID (uint8_t ri)      {
  ...
  if (readerConnected)    {
    ...
    if (isoType == TYPE14443PN532)     {  //PN532
      if (TIMING) timeStart = millis();
      cardFound = nfc14443_532.readPassiveTargetID(PN532_MIFARE_ISO14443A, &nuidHex[0], &numBytesRead, 35); //SET TIMEOUT (ORIG = no timeout) Won't work at leass than 30
      if (cardFound & TIMING) Serial << "532Found: " << loopCnt << " LS: " << loopStartTime << " lST-tS: " << loopStartTime - timeStart << " Now-tS: " << millis() - timeStart << endl;
    ...
    if (isoType == TYPE14443PN5180)     {
      ...
      if (cardReadResult >= 4) {             //Minium length for Type A uid
        if (TIMING) {
          jmri[ri].JMRIMemory[1] = String(timeStart-loopStartTime);
          Serial << "5180-43Found: " << loopCnt << " LS: " << loopStartTime << " tS-lSt: " << jmri[ri].JMRIMemory[1] << " Now-tS: " << millis() - timeStart << endl;
          MQTTSendMemory(ri,1);
        }
        cardFound = true;
      ...
      if (rc == ISO15693_EC_OK) {
        if (TIMING) Serial << "5180-93Found: " << loopCnt << " LS: " << loopStartTime << " lST-tS: " << loopStartTime - timeStart << " Now-tS: " << millis() - timeStart << endl;
        cardFound = true;
    ...
    if (isoType == TYPE14443RC522)   {
      ...
      if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
        if (TIMING) Serial << "522Found: " << loopCnt << " LS: " << loopStartTime << " lST-tS: " << loopStartTime - timeStart << " Now-tS: " << millis() - timeStart << endl;
        cardFound = true;
...
void ManualReboot()    {               //CAN BE REQUESTED VIA MQTT MESSAGE OR AS A RESULT OF NOT CONNECTING TO MQTT
  ...
  Serial.println("**E**527c: Manual REBOOT -- check for OTA update first");
