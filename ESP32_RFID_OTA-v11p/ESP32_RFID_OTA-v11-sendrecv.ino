...
void LightRcvd (uint8_t lNum, String lState) {       //lNum = JMRI Light number (used for RFID reader re-init)
  ...
  if (lNum == ACK_SENS_NUM)  {                       //Light = [large number] is JMRI Activate switch
    if (lState == "ON")         {
      ...
      mqttclient.publish(mqttChannel+mqttPublishTopics[SS_MEM]+String(mqttClientID)+JMRI_MEM_ESP32_IP, WiFi.localIP().toString(), true, 0);  //Retaned = true
      //Update the VERSIONNUM memory in JMRI
      mqttclient.publish(mqttChannel+mqttPublishTopics[SS_MEM]+String(mqttClientID)+JMRI_MEM_ESP32_VERS, VERSIONNUM, true, 0);  //Retaned = true
